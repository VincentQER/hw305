...
public class ClockTests {
	...
	private static void testSetToCurrentTime() {
		System.out.println("CLOCK SETTOCURRENTTIME TESTS");

		Clock myClock = new Clock(); // Create new clock object
		...
		Clock comparedClock = new Clock(hours, minutes, seconds);
		System.out.println("Testing time...");

		if (myClock.compareTo(comparedClock) == 0)
			System.out.println("\tPASSED current time is valid");
		else
			System.out.println("\tFAILED current time is not valid: " + myClock);
		System.out.println("-------------------------------------------");
	}
